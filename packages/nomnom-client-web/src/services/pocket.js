...
function pocketLinkToEntry(status, link) {
  return {
    ...
    creationDate: new Date(
      parseInt(link.attributes.time_added.value, 10) * 1000
    ),
    tags: link.attributes.tags.value
      ? link.attributes.tags.value.split(",")
      : []
  };
}
...
function importPocketExport(file) {
  return readFileAsText(file).then(fileContent => {
    ...
    const unreadEntries = Array.from(
      unreadList.querySelectorAll("a")
    ).map(link => pocketLinkToEntry("LATER", link));
